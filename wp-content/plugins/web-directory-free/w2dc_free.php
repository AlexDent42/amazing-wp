...
define('W2DC_MAPBOX_VERSION', 'v1.12.0');
...
class w2dc_plugin {
	...
	public function pre_get_posts($query) {
		...
		if ($query->is_main_query()) {
			if (w2dc_isListing() || w2dc_isCategory() || w2dc_isLocation() || w2dc_isTag()) {
				if (w2dc_isListing()) {
					$query->set('page_id', $this->listing_page_id);
				} else {
					$query->set('page_id', $this->index_page_id);
				}
	...
	public function loadNestedFrontendController($pattern, $content) {
		...
		do_action("w2dc_load_frontend_controllers");
	}
	...
	public function buildRules($lang_code = '') {
		...
		$rules['(' . $page_url . ')?/?' . $listing_slug . '/([^\/.]+)/reviews/([^\/.]+)/?' . $lang_param . '$'] = 'index.php?page_id=' . $review_page_id . '&target-post=$matches[2]&review-w2rr=$matches[3]&' . $listing_param;
		// /%listing_slug%/%postname%/reviews/page/%n%/
		$rules['(' . $page_url . ')?/?' . $listing_slug . '/([^\/.]+)/reviews/' . $wp_rewrite->pagination_base . '/([0-9]{1,})/?' . $lang_param . '$'] = 'index.php?page_id=' . $review_page_id . '&target-post=$matches[2]&reviews-all=1&paged=matches[3]&' . $listing_param;
		// /%listing_slug%/%postname%/reviews/
		$rules['(' . $page_url . ')?/?' . $listing_slug . '/([^\/.]+)/reviews/?' . $lang_param . '$'] = 'index.php?page_id=' . $review_page_id . '&target-post=$matches[2]&reviews-all=1&' . $listing_param;
		...
		$rules[$page_url . '/([^\/.]+)/reviews/([^\/.]+)/?' . $lang_param . '$'] = 'index.php?page_id=' . $review_page_id . '&target-post=$matches[1]&review-w2rr=$matches[2]&' . $listing_param;
		...
		strpos(get_option('permalink_structure'), '/%year%/%postname%') === FALSE
			...
			$rules['(' . $page_url . ')?/?(?!(?:199[0-9]|20[012][0-9])/(?:0[1-9]|1[012]))([0-9]+)/([^\/.]+)/reviews/([^\/.]+)/?' . $lang_param . '$'] = 'index.php?page_id=' . $review_page_id . '&target-post=$matches[3]&review-w2rr=$matches[4]&' . $listing_param;
		...
		$rules['(' . $page_url . ')?/?' . $listing_slug . '/(.+?)/([^\/.]+)/reviews/([^\/.]+)/?' . $lang_param . '$'] = 'index.php?' . $page_param . 'tax-slugs-w2dc=$matches[2]&target-post=$matches[3]&review-w2rr=$matches[4]&' . $listing_param;
	...
	public function listing_permalink($permalink, $post, $leavename) {
		if ($post->post_type == W2DC_POST_TYPE) {
			...
			if ($wp_rewrite->using_permalinks()) {
				...
				return w2dc_templatePageUri(array(W2DC_POST_TYPE => $post->post_name), $listing_page_url);
			}
		}
		return $permalink;
	}
	...
	public function category_permalink($permalink, $category, $tax) {
		if ($tax == W2DC_CATEGORIES_TAX) {
			...
			if ($wp_rewrite->using_permalinks()) {
				...
				return w2dc_templatePageUri(array(W2DC_CATEGORIES_TAX => $category->slug), $this->index_page_url);
		}
		return $permalink;
	}
	...
	public function suspend_expired_listings() {
		...
		$posts_ids = $wpdb->get_col($wpdb->prepare("
				...
				foreach ($listings_ids_to_suspend AS $listing_id) {
					...
					if (w2dc_isListingPayment($listing_id)) {
		...
		if ($listing = $this->listings_manager->loadListing($listing_id)) {
			...
			wp_enqueue_style('w2dc-jquery-ui-style');
				...
				wp_enqueue_style('w2dc_mapbox_gl');
					...
					wp_register_script('w2dc_mapbox_directions', W2DC_RESOURCES_URL . 'js/mapbox-gl-directions-w-language.js', array('jquery'), W2DC_VERSION_TAG, true);
					wp_enqueue_script('w2dc_mapbox_directions');
					wp_register_style('w2dc_mapbox_directions', 'https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-directions/v4.0.0/mapbox-gl-directions.css');
