<script>
	var w2dc_locations_metabox_attrs = {
		post_id: <?php echo $listing->post->ID; ?>,
		locations_number: <?php echo $listing->level->locations_number; ?>,
		is_map_markers: <?php echo ($listing->level->map && $listing->level->map_markers) ? 1 : 0; ?>,
		is_map: <?php echo ($listing->level->map) ? 1 : 0; ?>,
		images_dialog_title: '<?php echo esc_js(__('Select map marker icon', 'W2DC')); ?>',
		icons_dialog_title: '<?php echo esc_js(__('Select map marker icon', 'W2DC') . ((get_option('w2dc_map_markers_type') == 'icons') ? __(' (icon and color may depend on selected categories)', 'W2DC') : '')); ?>'
	}
</script>
