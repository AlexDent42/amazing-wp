...
function w2dc_comments_open() {
	...
	$comments_open = apply_filters('w2dc_comments_open', $comments_open);
...
function w2dc_visibleSearchParam($param_text, $link) {
	$parse_url = parse_url($link, PHP_URL_QUERY);
	parse_str($parse_url, $parse_url_str);
	if (count($parse_url_str) == 1 && w2dc_getValue($parse_url_str, 'w2dc_action') == 'search') {
		$link = remove_query_arg('w2dc_action', $link);
	} elseif ($use_advanced = w2dc_getValue($_REQUEST, 'use_advanced')) {
		$link = add_query_arg('use_advanced', '1', $link);
	}
	
	return '<div class="w2dc-search-param"><a class="w2dc-search-param-delete" href="' . $link . '">×</a>' . esc_html(stripslashes($param_text)) . '</div>';
}
