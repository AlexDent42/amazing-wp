...
class w2dc_listings_manager {
	...
	public function listingExpirationDateMetabox($post) {
		...
		if ($listing->status != 'expired') {
			...
			w2dc_renderTemplate('listings/change_date_metabox.tpl.php', array('listing' => $listing, 'dateformat' => w2dc_getDatePickerFormat()));
		} else {
	...
	public function changeDateListingPage() {
		if (isset($_GET['listing_id']) && ($listing_id = $_GET['listing_id']) && is_numeric($listing_id) && w2dc_current_user_can_edit_listing($listing_id)) {
			if ($this->loadCurrentListing($listing_id)) {
				...
				w2dc_renderTemplate('listings/change_date.tpl.php', array('listing' => $this->current_listing, 'referer' => $referer, 'action' => $action, 'dateformat' => w2dc_getDatePickerFormat()));
			} else
				exit();
		} else
			exit();
